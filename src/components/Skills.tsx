      ...
      <div className="container mx-auto px-6">
        ...
        <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-6">
          {skills.map((skill, index) => (
            <RevealItem key={skill.name} index={index}>
              <div className="glass-card card-glow p-6 rounded-xl hover:-translate-y-1 transition-all duration-300 group">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-lg font-semibold text-foreground group-hover:text-primary transition-colors">{skill.name}</h3>
                  <span className="text-sm text-primary font-bold bg-primary/10 px-2.5 py-0.5 rounded-full">{skill.level}%</span>
                </div>
